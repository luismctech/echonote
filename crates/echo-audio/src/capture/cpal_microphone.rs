...
fn enumerate_input_devices() -> Result<Vec<DeviceInfo>, DomainError> {
    ...
    let mut out = Vec::new();
    for dev in devices {
        let name = dev.description().map(|d| d.name().to_string()).unwrap_or_else(|_| "<unnamed>".to_string());
        out.push(DeviceInfo {
            id: name.clone(),
...
fn start_capture(spec: &CaptureSpec) -> Result<StartedStream, DomainError> {
    ...
    let device_name = device.description().map(|d| d.name().to_string()).unwrap_or_else(|_| "<unnamed>".to_string());
...
fn pick_device(host: &cpal::Host, device_id: Option<&str>) -> Result<Device, DomainError> {
    if let Some(id) = device_id {
        let devices = host
            ...
            .map_err(|e| DomainError::AudioDeviceUnavailable(format!("input_devices: {e}")))?;
        for dev in devices {
            if dev.description().ok().map(|d| d.name().to_string()).as_deref() == Some(id) {
                return Ok(dev);
            }
        }
